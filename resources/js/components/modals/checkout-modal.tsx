...
const CheckoutModal: React.FC<CheckoutModalProps> = ({
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            // Create transaction record in database using context
            ...
            // Use the context hook for transaction creation
            const result = await createNewTransaction(transactionData);

            if (result?.success && result.data?.transaction) {
                // Notify parent component that transaction was created
                onTransactionCreated?.();
                
                // Prepare transaction data for invoice template
                const createdTransaction: TransactionData = {
                    id: result.data.transaction.id || Date.now(),
                    order_id: result.data.transaction.order_id || `INV-${Date.now()}`,
                    customer_id: selectedCustomer?.id || 0,
                    amount: grandTotal,
                    customer: {
                ...
                // Send email with invoice template if requested
                if (sendEmail && selectedCustomer?.email) {
                    try {
                        await sendOrderEmailWithInvoice(createdTransaction);
                    } catch (error) {
                        toast({
            ...
            // If no phone is available, show a warning but continue with the checkout
            ...
            if (sendWhatsApp && selectedCustomer?.phone && selectedCustomer.phone.trim() !== '') {
                try {
                    await sendInvoiceViaWhatsApp(createdTransaction);
                } catch (error: unknown) {
                    toast({
                        title: t('pos.error.whatsappFailed'),
                ...
                toast({
                    title: t('pos.checkout.orderCompleted'),
                    description: result.data?.transaction?.order_id 
                        ? t('toast.orderProcessed', { orderId: result.data.transaction.order_id })
                        : t('toast.orderCompleted'),
                    variant: 'success',
                });
            }
            ...
            <div className="bg-white dark:bg-[#1a1a1a] rounded-lg sm:rounded-xl shadow-2xl dark:shadow-black/50 w-full max-w-6xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden border border-gray-200 dark:border-gray-800">
                ...
                <div className="p-3 sm:p-4 overflow-y-auto max-h-[calc(95vh-120px)] sm:max-h-[calc(90vh-140px)]">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
                        ...
                        <div className="lg:col-span-1 space-y-3">
                            ...
                            <div className="bg-white dark:bg-[#1a1a1a] rounded-lg p-3 border border-gray-200 dark:border-gray-800 shadow-sm">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <h3 className="font-medium text-gray-900 dark:text-white text-sm sm:text-base">{t('pos.checkout.paymentStatus')}</h3>
                                </div>
                                <div className="space-y-2 sm:space-y-3">
                                    <label className={`flex items-center gap-2 sm:gap-3 cursor-pointer p-2 rounded-lg transition-all duration-200 ${
                                        isPaid 
                                        ...
                                        <div className="relative flex-shrink-0">
                                            ...
                                            <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-md border-2 transition-all duration-200 flex items-center justify-center ${
                                                isPaid
                                                    ...
                                                    : 'bg-white dark:bg-[#1a1a1a] border-gray-300 dark:border-gray-800 hover:border-emerald-400 dark:hover:border-emerald-500'
                                            }`}>
                                                {isPaid && (
                                                    <svg 
                                                        className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" 
                                        ...
                                        <div className="flex items-center gap-1 sm:gap-2 min-w-0 flex-1">
                                            <DollarSign className={`w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 ${isPaid ? 'text-emerald-600' : 'text-gray-500'}`} />
                                            <span className={`text-xs sm:text-sm font-medium truncate ${
                                                isPaid 
                                                    ? 'text-emerald-700 dark:text-emerald-300' 
                                                    : 'text-gray-700 dark:text-gray-300'
                                            }`}>
                                                {isPaid ? t('pos.checkout.paid') : t('pos.checkout.markAsPaid')}
                                            </span>
                                        </div>
                                    </label>
                                    ...
                                    {!isPaid && (
                                        <div className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 p-2 rounded-md border border-amber-200 dark:border-amber-800">
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="font-medium">{t('pos.checkout.debt')}</span>
                                            </div>
                                            <p className="mt-1 text-amber-700 dark:text-amber-300">
                                                {t('pos.checkout.debtWarning')}
                                            </p>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="bg-white dark:bg-[#1a1a1a] rounded-lg p-3 border border-gray-200 dark:border-gray-800 shadow-sm">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                                    ...
                                    <button
                                        ...
                                        className="px-3 py-1.5 text-xs sm:text-sm bg-[#F58E18] hover:bg-[#EA580C] text-white rounded-lg transition-colors duration-200 cursor-pointer self-start sm:self-auto shadow-sm hover:shadow-md"
                                    >
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex flex-col sm:flex-row gap-2">
                                            <input
                                                ...
                                                className="flex-1 px-3 py-2 text-sm border border-gray-300 dark:border-gray-800 rounded-lg bg-white dark:bg-[#1a1a1a] text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-[#F58E18] focus:border-[#F58E18]"
                                            />
                                            <button
                                                onClick={() => setDiscountAmount(null)}
                                                className="px-3 py-2 text-xs sm:text-sm border border-gray-300 dark:border-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#262626] rounded-lg transition-colors cursor-pointer whitespace-nowrap"
                                            >
                        ...
                        <div className="lg:col-span-2 space-y-3">
                                                        ...
                                                        <td className="px-3 py-2 text-right">
                                                            <div className="space-y-1">
                                                                <input
                                                                    ...
                                                                    placeholder="0"
                                                                    className="w-20 px-2 py-1 text-sm border border-gray-300 dark:border-gray-800 rounded bg-white dark:bg-[#1a1a1a] text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-1 focus:ring-[#3b82f6] focus:border-[#3b82f6]"
                                                                />
                                                                {productDiscount > 0 && (
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="pt-2 border-t border-gray-100 dark:border-gray-800">
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder="0"
                                                        className="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-800 rounded bg-white dark:bg-[#1a1a1a] text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-1 focus:ring-[#3b82f6] focus:border-[#3b82f6]"
                                                    />
                                                </div>
                                            </div>
                                        </div>
                            ...
                            {/* Grand Total */}
                            <div className={`rounded-lg p-3 border bg-white dark:bg-[#1a1a1a] ${
                                isPaid 
                                    ? 'border-[#10b981] dark:border-[#10b981]'
                                    : 'border-[#3b82f6] dark:border-[#3b82f6]'
                            }`}>
                                <div className="space-y-2">
                                    {/* Subtotal */}
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-2 border-t border-gray-200 dark:border-gray-800 gap-2">
                                        <div className="flex-1">
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                                                ...
                                                {isPaid ? (
                                                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300 self-start">
                                                        <DollarSign className="w-3 h-3" />
                                                        {t('pos.checkout.paid')}
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300 self-start">
                                                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                                                        </svg>
                                                        {t('pos.checkout.debt')}
                ...
                <div className="border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1a1a1a] p-3 sm:p-4 shadow-sm">
                    <div className="flex flex-row gap-2 sm:gap-3">
                        <button
                            onClick={onClose}
                            className="flex-1 px-3 py-2 sm:px-4 sm:py-2.5 border border-gray-300 dark:border-gray-800 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#262626] transition-colors font-medium cursor-pointer text-xs sm:text-base touch-manipulation"
                        >
                        ...
                        >
                            {isProcessing ? (
                                <>
                                    ...
                                    <span className="hidden sm:inline">{t('pos.checkout.processingOrder')}</span>
